...
'스프링프레임워크', 38000, 8,'김기태','정복사','2022-09-10');
...
update sales set 
money=(select bookprice from book where 
bookid=sales.bno)*amount;
...
alter table member2 modify id varchar(12) primary key; -- 또는
alter table member2 add constraints idkey primary key (id);
-- alter table member2 drop constraints idkey;
...
alter table book2 add constraint bfk2 foreign key (bookkind)
references bookkind(kindcode);
...
select distinct id,password,name from member where id not in (select id from sales);
...
where bookprice>=19000 and bookprice<=30000;    -- 또는
select booktitle, bookprice, author from book 
...
where pubcom='한빛미디어' or pubcom='남가람북스';
...
-- select * from book where pubdate like '2022%'; varchar인 경우 가능
...
-- 오름차순 asc : 작은 것에서 부터 큰 것순으로
-- 내림차순 desc : 큰 것에서 부터 작은 것순으로
...
select id,count(*) as "구매건수" from sales
...
-- 44) 44) 판매 뷰(sales_view)를 생성하되 판매(sales) 테이블을 활용하고, 판매코드(sno), 도서코드(bno), 아이디(id), 판매금액(money) 컬럼만 추출되어 생성되게 하시오.
create view sales_view as 
select sno,bno,id,money from sales;
...
from member a inner join sales b on a.id=b.id;
...
minus 
select distinct a.booktitle, a.bookprice, a.author 
from book a right outer join sales b on a.bookid=b.bno;  

select booktitle, bookprice, author from book where bookid in 
(select bookid from book minus select bno from sales);
...
minus 
select distinct a.id, a.name from 
member a right outer join sales b 
on a.id=b.id;
...
(select sno,bno,id,amount,money from sales 
UNION select sno,bno,id,amount,money from sales2);
...
INTERSECT select id, name, address from member2);
